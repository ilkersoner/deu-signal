...
void gcreate(char* name,Client user);
void join(char* name,Client user);
...
{  
    ...
    char msg[60]= "Enter your username: ";
    ...
    strcpy(msg,"\nWelcome to DEU Signal Lobby! Enter a command\n");
    ...
    while(1)
    {
      if(strncmp(command,"-gcreate",8)==0)
       {gcreate(command,user);}
...
void gcreate(char* name,Client user)
{
    ...
    for(i=0;i<9;i++)
      name++;
    int cont;            // Enter , create , gcreate are similar.
    cont = 1;
    for(i=0;i<ROOMCOUNT;i++){
      if(strcmp(rooms[i].name,"") !=0 && strcmp(rooms[i].name,name)==0)
       { cont=0;  }
...
void preparerooms()
{
   strcpy(rooms[0].name,"Computer");
   strcpy(rooms[1].name,"Endustry");
   strcpy(rooms[2].name,"Mechanical");
   strcpy(rooms[3].name,"Electrical");  // Prepare main rooms
    ...
    if(bind(socket_desc, (struct sockaddr *)&server, sizeof(server)) < 0)
{
    ...
    for(j=0;j<ROOMCOUNT;j++){
      sem_init(&getin[j],0,1);   // initial semaphores.
